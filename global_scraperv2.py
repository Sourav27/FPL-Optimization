...
from getters import *
from collectorv2 import collect_gw, merge_gw
...
def parse_data():
    ...
    #     parse_player_gw_history(player_data["history"], player_base_filename, name, i)
        if gw_num > 0:
            ...
            with open(os.path.join(gw_base_filename, 'xP' + str(gw_num) + '_.csv'), 'w+', newline='') as outf:
                w = csv.DictWriter(outf, ['id', 'xP','xP2'])
                w.writeheader()
                for xp in xPoints:
                    w.writerow(xp)
