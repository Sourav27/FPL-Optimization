...
def get_expected_points(gw, directory):
    ...
    xPoints_next = {}
    try:
        fin = open(os.path.join(directory, 'xP' + str(gw) + '_.csv'), 'rU')
        reader = csv.DictReader(fin)
        for row in reader:
            xPoints[int(row['id'])] = row['xP']
            xPoints_next[int(row['id'])] = row['xP2']
    except:
        return xPoints, xPoints_next    
    return xPoints, xPoints_next
...
def collect_gw(gw, directory_name, output_dir, root_directory_name="data/2022-23"):
    ...
    xPoints, xPoints_next = get_expected_points(gw, output_dir)
            ...
            if fname == "gw.csv":
                ...
                for row in reader:
                    if int(row['round']) == gw:
                        ...
                        if id in xPoints:
                            row['xP'] = xPoints[id]
                            row['xP2'] = xPoints_next[id]
                        else:
                            row['xP'] = 0.0
                            row['xP2'] = 0.0
                        rows += [row]

    fieldnames = ['name', 'position', 'team', 'xP', 'xP2'] + fieldnames
    outf = open(os.path.join(output_dir, "gw" + str(gw) + "_.csv"), 'w', encoding="utf-8")
    writer = csv.DictWriter(outf, fieldnames=fieldnames, lineterminator='\n')
    writer.writeheader()
    for row in rows:
        writer.writerow(row)

def collect_all_gws(directory_name, output_dir, root_dir="data/2022-23"):
    for i in range(1,38):
...
def main():
    ...
    collect_all_gws(player_base_filename, gw_base_filename, base_filename)
    # collect_gw(gw_num, player_base_filename, gw_base_filename, base_filename) 
    merge_all_gws(38, gw_base_filename)
